 ...
 *
 * Fixed by: Nazario D'Apote (05/04/2020)
 */
...
(function (root, JQuery, factory) {
    if (typeof define === "function" && define.amd) {
        ...
        define(['jquery'], function ($) {
            factory($, root.window, root.document, undefined);
        });
    } else if (typeof module === "object" && module.exports) {
        // CommonJS
        factory(root.require('jquery'), root.window, root.document, undefined);
    } else {
        // Browser globals
        factory(JQuery, root.window, root.document, undefined);
    }
    ...
    var MessageBoxDialog = {
        ...
        hideButtons: function ($dialog) {
            ...
            //});
            var self = this;
            if (self.options.NoButtonText.length === 0) {
                $dialog.find('#btn-no').each(function (index, item) {
                    $(item).hide();
                });
            }
            if (self.options.okButtonText.length === 0) {
                $('#btn-accept').each(function (index, item) {
                    $(item).hide();
                });
            }
            if (self.options.closeButtonText.length === 0) {
